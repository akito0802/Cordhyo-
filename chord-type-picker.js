...
(()=>{
 ...
 wrap.querySelector('.type-more').onclick=e=>{panel.hidden=!panel.hidden;e.currentTarget.textContent=panel.hidden?'もっと見る':'閉じる';if(!panel.hidden){renderAll();input.focus();}};
 input.oninput=()=>renderAll(input.value);sel.addEventListener('change',render);
 const st=document.createElement('style');st.textContent=`.type-picker{margin:0 0 8px;padding:8px 9px;border:1px solid #ded6c9;border-radius:12px;background:#fff}.type-row{display:flex;align-items:center;gap:8px}.type-row>b{font-size:.72rem;color:#665b4f;flex:0 0 auto}.type-cats{display:flex;gap:4px;overflow-x:auto;scrollbar-width:none;min-width:0}.type-cats::-webkit-scrollbar{display:none}.type-cats button{flex:0 0 auto;border:1px solid #ddd0bf;background:#fffaf3;color:#4a4036;border-radius:8px;padding:6px 8px;font-size:.68rem;font-weight:800}.type-cats button.active{background:#e9dfd0;border-color:#aa9a85}.type-buttons{display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin-top:7px}.type-buttons button,.type-all button{border:1px solid #d9cdbd;background:#fffaf3;color:#4a4036;border-radius:8px;font-weight:800;min-height:34px;font-size:.76rem}.type-buttons button.active,.type-all button.active{background:#3f382f;color:#fff;border-color:#3f382f}.type-bottom{display:flex;justify-content:space-between;align-items:center;margin-top:6px}.type-current{font-size:.68rem;color:#786d60;font-weight:800}.type-more{border:0;background:transparent;color:#675c50;font-size:.68rem;font-weight:800;padding:4px}.type-panel{margin-top:7px;border-top:1px solid #eee4d7;padding-top:7px}.type-panel input{width:100%;box-sizing:border-box;border:1px solid #d7cbbb;border-radius:8px;padding:8px;font-size:14px}.type-all{display:grid;grid-template-columns:repeat(3,1fr);gap:5px;max-height:260px;overflow:auto;margin-top:7px}.type-all button{font-size:.68rem;padding:4px 2px}@media(min-width:700px){.type-buttons,.type-all{grid-template-columns:repeat(6,1fr)}}`;document.head.appendChild(st);render();
})();