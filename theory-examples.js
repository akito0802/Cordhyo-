...
const categorySongs={
 basics:['「Ode to Joy」','音の上がり下がり、音程、主音へ戻る感覚を確認する'],
 chords:['「Let It Be」','コード構成音、転回形、付加音による響きの違いを聴く'],
 harmony:['「Autumn Leaves」','機能和声、終止、循環進行、ガイドトーンの動きを追う'],
 scaleMelody:['「So What」','スケールの特徴音とメロディの重心を意識して聴く'],
 composition:['「Pretender」','セクション構成、音域、和声密度、メロディ展開を追う'],
 guitar:['「Little Wing」','コードフォーム、装飾音、ポジション移動をギターで確認する'],
 ear:['「Stand by Me」','ベース、コード、メロディを分けて聴き取る'],
 jazz:['「Fly Me to the Moon」','ii–V–I、7th、テンション、ボイシングの流れを追う'],
 rhythm:['「Billie Jean」','反復するリズム、アクセント、グルーヴの位置を聴く'],
 arrangement:['「Bohemian Rhapsody」','楽器編成、音域配置、場面転換を追う'],
 genre:['「Come Together」','ジャンル特有のリズム、ベース、コード感を聴く']
};
const catDefaults={
...
function build(card){if(card.dataset.examplesAdded)return;const title=titleOf(card),found=findItem(title);if(!found)return;card.dataset.examplesAdded='1';const {cat,item}=found,d=catDefaults[cat]||catDefaults.basics,song=songMap[title]||categorySongs[cat]||['「Let It Be」','コード・メロディ・ベースの関係を意識して聴く'];const example=item.example||'実際に音を鳴らし、理論上の説明と耳で感じる印象を比べる。';const points=(item.points||[]).slice(0,3);
