...
let audioBuffer=null,peaks=null,raf=0;
...
async function load(file){if(!file)return;try{if(ctx.state==='suspended')await ctx.resume();const data=await file.arrayBuffer();audioBuffer=await ctx.decodeAudioData(data.slice(0));peaks=makePeaks(audioBuffer);stage.classList.add('ready')}catch(e){audioBuffer=null;peaks=null;stage.classList.remove('ready')}}
